      ...
      <van-list v-model="loading"
        ...
        <div class="f-flex zyitem box-shadow"
          ...
          <div>
            ...
            <p>{{$t('m.Publish.task5')}}：<br>{{item.needs}}</p>
            <p v-show="item.uid=='0'?false:true">{{$t('m.Publish.task6')}}：{{item.mobile}}</p>
          </div>
          <div>
            <p>{{$currency}} {{item.money}}{{$numberUnit}}</p>
            <!--{{$t('m.Publish.task7')}} -->
            <p>
...
export default {
  ...
  methods: {
    ...
    confirms () {
      ...
      if (res.status == 0) {
        this.$toast(this.$t('m.Publish.task19'))
        setTimeout(() => {
          this.$router.go(0)
        }, 500)
      }else if(res.status == 3){
        this.$toast(this.$t('m.Publish.task20'))
        setTimeout(() => {
          this.$router.go(0)
        }, 500)
      }else if(res.status == 4){
        this.$toast(this.$t('m.Publish.task21'))
        setTimeout(() => {
          this.$router.go(0)
        }, 500)
      }else if(res.status == 5){
        this.$toast(this.$t('m.Publish.task22'))
        setTimeout(() => {
          this.$router.go(0)
        }, 500)
      }else if(res.status == 6){
        this.$toast(this.$t('m.Publish.task23'))
        setTimeout(() => {
          this.$router.go(0)
        }, 500)
      }else if(res.status == 7){
        this.$toast(this.$t('m.Publish.task24'))
        setTimeout(() => {
          this.$router.go(0)
        }, 500)
      }else if(res.status == 8){
        this.$toast(this.$t('m.Publish.task25'))
        setTimeout(() => {
          this.$router.go(0)
        }, 500)
